...
const OnlineBadge = ({ socketId }) => (
  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${
    socketId ? "bg-green-500/15 text-green-400 border border-green-500/25" : "bg-zinc-700/50 text-gray-500 border border-white/10"
  }`}>
    <span className={`w-1.5 h-1.5 rounded-full ${socketId ? "bg-green-400 animate-pulse" : "bg-gray-600"}`} />
    {socketId ? "Online" : "Offline"}
  </span>
);

const rideStatusLabel = (status) => {
  if (status === "pending") return "Not Ride Booked";
          ...
          <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <OnlineBadge socketId={detail?.user?.socketId} />
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                {
                  label: "Total Rides",
                  value: detail?.stats?.totalRides ?? 0,
                  color: "text-amber-400",
                },
                {
                  label: "Completed",
                  value: detail?.stats?.completedRides ?? 0,
                  color: "text-green-400",
                },
                {
                  label: "Cancelled",
                  value: detail?.stats?.cancelledRides ?? 0,
                  color: "text-red-400",
                },
              ].map((s) => (
                <div
              ...
              <div>
                ...
                <div className="space-y-2">
                  {detail.recentRides.map((r) => (
                    <div
                      key={r._id}
                      className="bg-white/5 border border-white/8 rounded-xl px-3 py-2.5 flex items-center justify-between"
                    >
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        <span className={`text-xs ${
                            ...
                            : r.status === "ongoing" ? "text-blue-400"
                            : "text-orange-400"
                          }`}>
  ...
  const fetchUsers = useCallback(async (silent = false) => {
    ...
    try {
      const r = await api.get("/users", {
        params: { page, search, filter, limit: 15 },
      });
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1">
                          <Badge blocked={u.isBlocked} />
                          <OnlineBadge socketId={u.socketId} />
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="w-8 h-8 rounded-lg bg-white/5 hover:bg-amber-500/15 hover:text-amber-400 text-gray-400 flex items-center justify-center transition text-sm"
                          >
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 flex items-center justify-center transition text-sm"
                          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-white/8">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400 text-xs disabled:opacity-40 hover:bg-white/10 transition"
                >
                  Prev
                </button>
                <button

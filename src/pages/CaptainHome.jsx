...
import CaptainNotification, { getAudioCtx } from "../components/CaptainNotification";
...
const CaptainHome = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_BASE_URL}/captains/wallet`, authHeader())
  ...
  const setOnlineStatus = useCallback(async (status) => {
    ...
    setIsOnline(status);
    try {
      await axios.post(`${import.meta.env.VITE_BASE_URL}/captains/toggle-online`, { isOnline: status }, authHeader());
    } catch {
      isOnlineRef.current = !status;
      setIsOnline(!status);
    }
  ...
  useEffect(() => {
    ...
    const emitJoin = () => socket.emit("join", { userId: captain._id, userType: "captain" });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      navigator.geolocation.getCurrentPosition(
        (pos) => socket.emit("update-location-captain", {
          userId: captain._id,
  ...
  const acceptRide = useCallback(async () => {
    ...
    setAcceptError("");
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/rides/confirm`, { rideId: ride._id }, authHeader());
      setShowNotification(false);
      ...
      if (data?.message === "insufficient_wallet") {
        ...
        setTimeout(() => { setShowNotification(false); setRide(null); setAcceptError(""); }, 2000);
      } else {
  ...
  const rejectRide = useCallback(() => { setShowNotification(false); setRide(null); setAcceptError(""); }, []);
  const handleLogout = useCallback(async () => { await setOnlineStatus(false); navigate("/captain-logout"); }, [setOnlineStatus, navigate]);
  const toggleOnline = useCallback(() => setOnlineStatus(!isOnlineRef.current), [setOnlineStatus]);

  return (
    <div className="h-screen w-full relative overflow-hidden bg-zinc-950" onClick={getAudioCtx}>
      ...
      <div className="absolute top-0 left-0 right-0 z-20 px-4 pt-safe pt-4">
        <div className="bg-zinc-900/90 backdrop-blur-xl rounded-2xl px-4 py-3 border border-white/10 shadow-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            {captain?.profilePic ? (
              <img src={`${import.meta.env.VITE_BASE_URL}/${captain.profilePic}`}
                className="w-9 h-9 rounded-full object-cover border-2 border-amber-500" alt="captain" />
            ) : (
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                <div className={`w-1.5 h-1.5 rounded-full ${isOnline ? "bg-green-400 shadow-[0_0_4px_#4ade80]" : "bg-gray-500"}`} />
        ...
        <div className="bg-zinc-900/95 backdrop-blur-2xl border-t border-white/10 rounded-t-3xl px-5 pt-5 pb-8 shadow-2xl">
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              {captain?.profilePic ? (
                <img src={`${import.meta.env.VITE_BASE_URL}/${captain.profilePic}`}
                  className="w-14 h-14 rounded-2xl object-cover border-2 border-amber-500/50" alt="captain" />
              ) : (
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  ...
                  <span className="text-gray-400 text-xs">{captain?.totalRides || 0} rides</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <p className="text-amber-400 font-black text-2xl">₹{captain?.earnings || "0"}</p>
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              { icon: "ri-roadster-line", value: captain?.totalRides || "0", label: "Rides" },
              { icon: "ri-star-line", value: captain?.rating || "5.0", label: "Rating" },
              { icon: "ri-shield-check-line", value: "Active", label: "Status" },
            ].map((stat, i) => (
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-3 text-center">
                <i className={`${stat.icon} text-amber-400 text-xl mb-1 block`}></i>
          ...
          <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-2xl px-4 py-3 mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-300 text-sm">{captain?.vehicle?.vehicleType || "Car"}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                  ...
                  <i className={`ri-wallet-3-fill text-base ${
              ...
              <div className="flex items-center gap-2">
                ...
                <div className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold border ${
                  isWalletLow
            ...
            {isWalletLow && (
              <div className="mt-2.5 bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2 text-left">
                <p className="text-red-400 text-xs font-semibold">
                  ⚠️ Min ₹{minBalance} required to accept rides · Add ₹{minBalance - (walletBalance || 0)} more
                </p>
              </div>
            )}
          </button>
          ...
          {isOnline && (
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <p className="text-gray-400 text-xs">Waiting for ride requests...</p>
            </div>
          )}
        </div>
      </div>
